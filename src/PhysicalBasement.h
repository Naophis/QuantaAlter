...
char validateStraight(float dist) {
	...
	if (!enableSensorCtrl)
		return true;
	return !(60 <= img_distance && img_distance <= 90);
}
...
float check_sen_error(void) {
	...
	if (Front_SEN.now < FRONT_OUT && validateStraight(img_distance)) {
		if (ABS(RS_SEN45.now - RS_SEN45.old) < kireme_R) {
			if (RS_SEN45.now > (RS_SEN45.ref - rightCtrlDiff)) {

//				if (globalState == BACK_STRAIGHT) {
...
//			Se2.error_now = check_sen_error_dia_side();
	...
	if (enableGPID) {
		// C.g = (Gyro.Kp * Gy.error_delta / dt + Gyro.Ki * Gy.error_now) * dt + C_old.g;

		C.g = (Gyro.Kp * Gy.error_now + Gyro.Ki * Gy.error_old + Gyro.Kd * Gy.error_delta) + (C_old.g - C_old2) * dt;
	} else {
...
float FB_velocity() {
	...
	if (enableVPID) {
		...
		C.v = (Vel.Kp * Velocity.error_now + Vel.Ki * Velocity.error_old + Vel.Kd * Velocity.error_delta) + (C_old.v - C_old2) * dt;
	}
	if (!enablePWM) {
		C.v = 0;
		C_old.v = 0;
